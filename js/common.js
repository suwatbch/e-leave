...
function initIndexShiftedit() {
  $G('static').addEvent("change", function () {
    var a = this.value.toInt();
    if (a != 0) {
      $E('workweek').style.display = null;
      $E('workweek').disabled = false;
    } else {
      $E('workweek').style.display = 'none';
      $E('workweek').disabled = true;
    }
  });
  
  var shifteditFieldsChanged = function () {
    var staticValue = $G('static').value.toInt();
    if (staticValue != 0) {
      $E('workweek').style.display = null;
      $E('workweek').disabled = false;
    } else {
...
function initSelectChange(module, ids) {
  for (var prop in ids) {
    $G(ids[prop]).addEvent('change', function() {
      ...
      loader.location('index.php?_' + q.join('&'));
    });
  }
}
...
function initEditProfile(prefix) {
  ...
  var elements = [$G('register_year')];
  elements.forEach(function (element) {
    if (element && $E('register_year').value != '') {
      element.addEventListener('change', function () {
        var params = new URLSearchParams({
          'year': $E('register_year').value,
          'member_id': $E('register_id').value
        }).toString();
        var url = WEB_URL + 'index.php/index/model/editprofile/quotasearch?' + params;
        send(url, '', function (xhr) {
          var ds = xhr.responseText.toJSON();
          if (ds) {
            $G('register_quota_leave1_id').value = ds.quota_leave1_id;
            $G('register_quota_leave1').value = ds.quota_leave1;
            $G('register_quota_leave2_id').value = ds.quota_leave2_id;
            $G('register_quota_leave2').value = ds.quota_leave2;
            $G('register_quota_leave3_id').value = ds.quota_leave3_id;
            $G('register_quota_leave3').value = ds.quota_leave3;
            $G('register_quota_leave5_id').value = ds.quota_leave5_id;
            $G('register_quota_leave5').value = ds.quota_leave5;
            $G('register_quota_leave7_id').value = ds.quota_leave7_id;
            $G('register_quota_leave7').value = ds.quota_leave7;
            $G('register_quota_leave8_id').value = ds.quota_leave8_id;
            $G('register_quota_leave8').value = ds.quota_leave8;
          } else if (xhr.responseText != '') {
            console.log(xhr.responseText);
          }
        });

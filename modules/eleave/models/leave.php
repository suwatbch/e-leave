...
class Model extends \Kotchasan\Model
{
    ...
    public function submit(Request $request)
    {
        ...
        // ตรวจสอบการเริ่มต้น session, token, และสิทธิ์สมาชิก
            ...
            if ($Items->status == $pStatusOld) {
                ...
                // เพิ่มรายการลาใหม่
                    ...
                    try {
                        ...
                        // ตรวจสอบว่าไม่ได้ลาซ้ำวัน
                        $start_date = $request->post('start_date')->date();
                        $end_date = $request->post('end_date')->date();
                        if ($this->isDuplicateLeaveDate($db, $login['id'], $start_date, $end_date, $request->post('start_time')->text(), $request->post('end_time')->text(), $request->post('start_period')->toInt())) {
                            $ret['alert'] = Language::get('Duplicate leave date found');
                        }
                         else {
                            // ไม่ได้เลือกการลา
                            if ($save['leave_id'] == 0) {
    ...
    // ฟังก์ชันย่อยเพื่อตรวจสอบการลาวันที่ซ้ำ
    // ฟังก์ชันย่อยเพื่อตรวจสอบการลาวันที่ซ้ำ
    private function isDuplicateLeaveDate($db, $member_id, $start_date, $end_date, $start_time, $end_time, $start_period)
    {
        // ตรวจสอบการลาทั้งวันในช่วงเวลา
        $status = [0,1,3];
        if ($start_period == 1) {
            // ตรวจสอบว่ามีการลาทั้งวันในช่วงเวลาที่กำหนดหรือไม่
            $fullDayLeave  = $db->createQuery()
                ->from('leave_items')
                ->where([
                    ['member_id', $member_id],
                    ['status', 'IN', $status],
                    ['start_date', '<=', $end_date], // วันเริ่มต้นการลาไม่เกินวันสิ้นสุดของการลาใหม่
                    ['end_date', '>=', $start_date], // วันสิ้นสุดการลาไม่ต่ำกว่าวันเริ่มต้นของการลาใหม่
                    ['start_period', '=', 0] // ตรวจสอบการลาทั้งวัน
                ])
                ->first();
    
            if ($fullDayLeave ) {
                return true; // พบว่ามีการลาทั้งวันในช่วงเวลาที่กำหนด
            }
            // ตรวจสอบช่วงเวลาที่ทับซ้อนกัน
            ...
            // ตรวจสอบการลาช่วงเวลาในช่วงวันทั้งหมด
            $overlappingLeave = $db->createQuery()
                ->from('leave_items')
                ->where([
                    ['member_id', $member_id],
                    ['status', 'IN', $status],
                    ['start_date', '<=', $end_date], // วันเริ่มต้นการลาไม่เกินวันสิ้นสุดของการลาใหม่
                    ['end_date', '>=', $start_date], // วันสิ้นสุดการลาไม่ต่ำกว่าวันเริ่มต้นของการลาใหม่
                    // ['start_period', '=', 1] // ตรวจสอบการลาช่วงเวลา
                ])
                ->first();

        if ($overlappingLeave) {
            return true; // พบว่ามีการลาทับซ้อนในช่วงเวลาที่กำหนด
        }
    
            // ตรวจสอบการลาทั้งวันซ้ำ
            return $db->createQuery()
                ->from('leave_items')
                ->where([
                    ['member_id', $member_id],

...
class Model extends \Kotchasan\Model
{
     ...
     */
    public function quotasearch(Request $request)
    {
        // รับค่า
        $queryParams = $request->getQueryParams();
        $year = (int)$queryParams['year'];
        $member_id = (int)$queryParams['member_id'];

        $res = [];

        $tableleave = \Kotchasan\Model::createQuery()
                        ->select('*')
                        ->from('leave')
                        ->cacheOn()
                        ->execute();

        foreach ($tableleave as $item) {
            // ลาป่วย
            if ($item->id == 1) {
                $res['quota_leave1_id'] = 0;
                $res['quota_leave1'] = $item->num_days;
            } else if ($item->id == 2) {
                $res['quota_leave2_id'] = 0;
                $res['quota_leave2'] = $item->num_days;
            } else if ($item->id == 3) {
                $res['quota_leave3_id'] = 0;
                $res['quota_leave3'] = $item->num_days;
            } else if ($item->id == 5) {
                $res['quota_leave5_id'] = 0;
                $res['quota_leave5'] = $item->num_days;
            } else if ($item->id == 7) {
                $res['quota_leave7_id'] = 0;
                $res['quota_leave7'] = $item->num_days;
            } else if ($item->id == 8) {
                $res['quota_leave8_id'] = 0;
                $res['quota_leave8'] = $item->num_days;
            }
        }

        $tablequota = self::getQuotaOfUser($year, $member_id);
        foreach ($tablequota as $item) {
            // ลาป่วย
            if ($item->leave_id == 1) {
                $res['quota_leave1_id'] = $item->id;
                $res['quota_leave1'] = $item->quota;
            } else if ($item->leave_id == 2) {
                $res['quota_leave2_id'] = $item->id;
                $res['quota_leave2'] = $item->quota;
            } else if ($item->leave_id == 3) {
                $res['quota_leave3_id'] = $item->id;
                $res['quota_leave3'] = $item->quota;
            } else if ($item->leave_id == 5) {
                $res['quota_leave5_id'] = $item->id;
                $res['quota_leave5'] = $item->quota;
            } else if ($item->leave_id == 7) {
                $res['quota_leave7_id'] = $item->id;
                $res['quota_leave7'] = $item->quota;
            } else if ($item->leave_id == 8) {
                $res['quota_leave8_id'] = $item->id;
                $res['quota_leave8'] = $item->quota;
            }
        }
            ...
            if (Login::notDemoMode($login)) {
                try {
                    ...
                    if ($isAdmin) {
                        ...
                        $quota = [];
                        $year = $request->post('register_year')->toInt();
                        $quota[] = array(
                            'leave_id' => 1,
                            'year' => $year,
                    ...
                    if ($user) {
                        ...
                        // ตรวจสอบและลบข้อมูลจาก shift_workdays
                        if ($isAdmin && $save['shift_id'] != 0) {
                            // ลบข้อมูลจาก shift_workdays โดยใช้ member_id ของ user
                            $db->delete($this->getTableName('shift_workdays'), array(array('member_id', $user['id'])), 0);
                        }

...
class Functions
{
     ...
     * @return array
     */
    public static function selectyear3($year)
    {
        $year_ofset = (int) Language::get('YEAR_OFFSET');
        $years = [];
        for ($i = $year - 1; $i <= $year + 1; $i++) {
            $years[$i] = $i + $year_ofset;
        }
        ksort($years);
        return $years;
    }
     ...
     * @return float
     */
    public static function sumdaystimepersent($days, $times)
    {
        $strtime = strval($times);
        $strtime = str_replace('.', '', $strtime);
        $floattime = (float) $strtime;
        if (strlen((string)$strtime) == 1 && $times != 0) {
            $strtime = $strtime . '0';
            $floattime = (100 * (float) $strtime) / 80;
            $floattime = str_replace('.', '', $floattime);
        }
        $days = strval($days) . '.' . strval($floattime);
        return (float)$days;
    }
    ...
    public static function getttotalleave($days, $times)
    {
        ...
        $result = $Tempdays . $Temptime;
        if ($result == '') {
            $result = '0 ' . Language::get('days');
        }
        return $result;
    }

    /**
     * @param float $num_days
     * @param float $days
     * @param float $times
     * @return string
     */
    public static function getleaveremain($num_days, $days, $times)
    {
        // กำหนดจำนวนชั่วโมงต่อวัน
        $hours_per_day = 8;
        $hour = $hours_per_day - $times;
        if ($times == 0) {
            $hour = 0;
        } else {
            $num_days -= 1;
        ...
        $Tempdays = $date == 0 ? '' : strval($date) . ' ' . Language::get('days');
        $Temptime = $hour == 0 ? '' : ' ' . strval($hour) . ' ' . Language::get('hours');
        $result = $Tempdays . $Temptime;
        if ($result == '') {
            $result = '0 ' . Language::get('days');
        }
    ...
    /**
     * แปลงวันที่จากรูปแบบ YYYY-MM-DD เป็น DD/MM/YYYY (พุทธศักราช)
     * รองรับทั้ง array และ string (คั่นด้วย comma)
     * @param array|string $dates วันที่ในรูปแบบ YYYY-MM-DD
     * @return array|string วันที่ในรูปแบบ DD/MM/YYYY (พุทธศักราช)
     */
    public static function convertDateToBuddhistEra($date)
    {
        $timestamp = strtotime($date);
        $buddhistYear = date('Y', $timestamp) + 543;
        return date('d/m/', $timestamp) . $buddhistYear;
    }
     ...
     */
    public static function convertDateToYYYYMMDD($date)
    {
